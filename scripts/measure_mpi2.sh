...
print_help() {
  cat <<'USAGE'
Usage:
  ./scripts/measure_mpi2.sh [--folder NAME] [--np 1,2,4,8]
                            [--iterations N] [--warmup N] [--reps N]
                            [--skip-build] [-h|--help]

Description:
  Runs MPI2 benchmark sweeps over process counts.
  For each np value, it executes:
    mpirun <binding> -np <np> ./src/ant_simu.exe --exec mpi2 --layout soa --benchmark --no-render ...
...
Outputs:
  results/<folder>/mpi2/soa/<timestamp>/
    run_config.env
    system_info.txt
    summary_np.csv
    np_<np>/
...
build_mpirun_binding_args() {
  local version
  version="$(mpirun --version 2>/dev/null | head -n 1 || true)"
  if [[ "${version}" == *"Open MPI"* ]]; then
    echo "--bind-to core --map-by core"
    return
  fi
  if [[ "${version}" == *"HYDRA"* || "${version}" == *"MPICH"* ]]; then
    echo "-bind-to core"
    return
  fi
  # Fallback: no explicit binding args if runtime is unknown.
  echo ""
}

SPECIFIC_FOLDER="test_mpi2_report_v1"
ITERATIONS=1200
WARMUP=200
REPS=5
SKIP_BUILD=0
...
OUT_DIR="${REPO_ROOT}/results/${SPECIFIC_FOLDER}/mpi2/soa/${RUN_ID}"
LATEST_LINK="${REPO_ROOT}/results/${SPECIFIC_FOLDER}/mpi2/soa/latest"

mkdir -p "${OUT_DIR}"
mkdir -p "${REPO_ROOT}/results/${SPECIFIC_FOLDER}/mpi2/soa"

BIND_ARGS_STR="$(build_mpirun_binding_args)"
# shellcheck disable=SC2206
BIND_ARGS=( ${BIND_ARGS_STR} )

{
  ...
  echo "mpirun_bind_args=${BIND_ARGS_STR}"
...
cat > "${SUMMARY_CSV}" <<'CSV'
np,reps,mean_k0_ms_iter,std_k0_ms_iter,mean_k1_ms_iter,std_k1_ms_iter,mean_k4_ms_iter,std_k4_ms_iter,mean_k5_ms_iter,std_k5_ms_iter,mean_k_mpi_sync_ms_iter,std_k_mpi_sync_ms_iter,mean_k_mpi_halo_ms_iter,std_k_mpi_halo_ms_iter,mean_k_mpi_migrate_ms_iter,std_k_mpi_migrate_ms_iter
CSV

echo "[2/4] Running MPI2 sweep: np=${NP_CSV}, reps=${REPS} ..."
for np in "${NP_LIST[@]}"; do
  ...
  cat > "${RAW_CSV}" <<'CSV'
rep,measured_iters,k0_ns,k1_ns,k4_ns,k5_ns,k_mpi_sync_ns,k_mpi_halo_ns,k_mpi_migrate_ns,k0_ms_iter,k1_ms_iter,k4_ms_iter,k5_ms_iter,k_mpi_sync_ms_iter,k_mpi_halo_ms_iter,k_mpi_migrate_ms_iter
CSV
  ...
  for rep in $(seq 1 "${REPS}"); do
    ...
    SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy OMP_NUM_THREADS=1 OMP_DYNAMIC=FALSE \
      mpirun "${BIND_ARGS[@]}" -np "${np}" "${BIN}" --benchmark --no-render --exec mpi2 --layout soa \
    ...
    k_mpi_sync_ns="$(metric_from_file k_mpi_sync_ns "${METRICS_FILE}")"
    k_mpi_halo_ns="$(metric_from_file k_mpi_halo_ns "${METRICS_FILE}")"
    k_mpi_migrate_ns="$(metric_from_file k_mpi_migrate_ns "${METRICS_FILE}")"
    ...
    k_mpi_sync_ms_iter="$(ns_per_iter_to_ms "${k_mpi_sync_ns}" "${measured_iters}")"
    k_mpi_halo_ms_iter="$(ns_per_iter_to_ms "${k_mpi_halo_ns}" "${measured_iters}")"
    k_mpi_migrate_ms_iter="$(ns_per_iter_to_ms "${k_mpi_migrate_ns}" "${measured_iters}")"

    echo "${rep},${measured_iters},${k0_ns},${k1_ns},${k4_ns},${k5_ns},${k_mpi_sync_ns},${k_mpi_halo_ns},${k_mpi_migrate_ns},${k0_ms_iter},${k1_ms_iter},${k4_ms_iter},${k5_ms_iter},${k_mpi_sync_ms_iter},${k_mpi_halo_ms_iter},${k_mpi_migrate_ms_iter}" \
      >> "${RAW_CSV}"
  done
  ...
  k_sync_stats="$(stats_for_column "${RAW_CSV}" "k_mpi_sync_ms_iter")"
  k_halo_stats="$(stats_for_column "${RAW_CSV}" "k_mpi_halo_ms_iter")"
  k_migrate_stats="$(stats_for_column "${RAW_CSV}" "k_mpi_migrate_ms_iter")"

  echo "${np},${REPS},$(split_stats "${k0_stats}" mean),$(split_stats "${k0_stats}" std),$(split_stats "${k1_stats}" mean),$(split_stats "${k1_stats}" std),$(split_stats "${k4_stats}" mean),$(split_stats "${k4_stats}" std),$(split_stats "${k5_stats}" mean),$(split_stats "${k5_stats}" std),$(split_stats "${k_sync_stats}" mean),$(split_stats "${k_sync_stats}" std),$(split_stats "${k_halo_stats}" mean),$(split_stats "${k_halo_stats}" std),$(split_stats "${k_migrate_stats}" mean),$(split_stats "${k_migrate_stats}" std)" \
    >> "${SUMMARY_CSV}"
done
