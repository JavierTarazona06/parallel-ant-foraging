...
\begin{table}[H]
\centering
\caption{Comparaison speedup mesuré vs speedup théorique (OpenMP SoA)}
\label{tab:omp-speedup-theory}
\begin{tabular}{@{}rrrr@{}}
\toprule
\textbf{Threads} & \textbf{\(S_{\text{mesuré}}(p)\)} & \textbf{\(S_{\text{th,idéal}}(p)\)} & \textbf{\(S_{\text{th,Amdahl}}(p)\)} \\
\midrule
1 & 1.000000 & 1.000000 & 1.000000 \\
2 & 1.567677 & 2.000000 & 1.507992 \\
4 & 1.882850 & 4.000000 & 2.021426 \\
8 & 2.436151 & 8.000000 & 2.436151 \\
\bottomrule
\end{tabular}
\end{table}

Cette comparaison montre deux lectures complémentaires:
\begin{itemize}[leftmargin=*]
  \item le modèle idéal \(S(p)=p\) reste une borne haute très optimiste;
  \item la borne d'Amdahl (calée ici sur \(S(8)\)) reproduit mieux la tendance mesurée,
  ce qui confirme une part non parallélisable non négligeable.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Comparaison explicite au théorique.}
  La Table~\ref{tab:omp-speedup-theory} confirme que le speedup mesuré s'écarte fortement du
  modèle idéal (à 8 threads: \(2.44\) vs \(8.00\)), alors qu'il reste proche d'une lecture
  Amdahl calibrée sur la campagne. Cela valide l'idée d'un gain réel mais borné par les coûts
  mémoire et les parties séquentielles résiduelles.
\end{itemize}
