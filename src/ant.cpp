...
{
    // Reuse the ant seed through small helper lambdas for random choice and direction.
    ...
    while ( consumed_time < 1. ) {
        ...
        // Choose the pheromone channel that matches the current loaded/unloaded state.
        ...
        double max_phen    = std::max( {phen( new_pos_ant.x - 1, new_pos_ant.y )[ind_pher],
                                     ...
                                     phen( new_pos_ant.x, new_pos_ant.y + 1 )[ind_pher]} );
        // Fall back to exploration when epsilon wins or no attractive pheromone is present.
        ...
        // Mark the destination cell before updating the ant internal state.
        ...
        // Update the ant position and handle nest/food state transitions.
