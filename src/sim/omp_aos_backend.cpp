...
OmpAosBackend::OmpAosBackend(std::vector<ant>& ants) : m_ants(ants) {}

const char* OmpAosBackend::name() const
{
    return "omp+aos";
}

void OmpAosBackend::step(WorldState& world, const SimConfig& sim_config)
{
...
std::unique_ptr<Renderer> OmpAosBackend::create_renderer(const fractal_land& land,
